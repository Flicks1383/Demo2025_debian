...
> [!WARNING]
> **Используйте `nmtui` для сетевой настройки и ни в коем случае не редактируйте `/etc/network/interfaces`, иначе отключите сервис `nmtui`, так как возникает конфликт между двумя сервисами настройки сети.**
...
Для настройки GRE туннеля, как и адресации, лучше выбрать `nmtui`
